...
from src.const.enums import CamerasEnum, ImageFormatsEnum, file_extension
from src.const.paths import wavelengths_dir, flat_field_filepath, flat_fields_dir, dark_field_filepath, dark_fields_dir, \
    metadata_dir
from src.loading.loading import load_from_json, get_item, load_wavelengths, load_image
...
class CameraInfo(BaseModel):
    ...
    def from_name(cls, camera_name: CamerasEnum) -> Self:
        cameras_config_path = metadata_dir() / "cameras.json"
        cameras_list = load_from_json(filepath=cameras_config_path)
        camera_info = get_item(items_list=cameras_list, key="name", value=camera_name)
